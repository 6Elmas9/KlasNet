...
export default function CombinedRecu({ eleve, paiements, classe, anneeScolaire, operateur }: CombinedRecuProps) {
  ...
  // aggregate allocations from paiements
  const attributedLabels: Record<number, string> = {};
  for (let pi = 0; pi < ((paiements || []).length); pi++) {
    const p: any = paiements[pi];
    ...
    if (allocs && Array.isArray(allocs) && allocs.length) {
      ...
      // try multiple attribution strategies when allocations are missing
      const anyP: any = p as any;
      let attributedEid: string | null = null;

      // 1) explicit modalite meta
      ...
      // 2) explicit typeFrais / versementIndex mapping
      if (!attributedEid) {
      ...
      // 3) fallback by matching montant to an échéance that still has remaining attendu
      if (!attributedEid) {
      ...
      if (attributedEid) {
        const ech = echeanceMap[attributedEid] || null;
        const label = ech ? (`V${ech.modalite || '?'} (${(Number(anyP.montant)||0).toLocaleString('fr-FR')} FCFA)`) : (`${attributedEid} (${(Number(anyP.montant)||0).toLocaleString('fr-FR')} FCFA)`);
        payéParEcheance[attributedEid] = (payéParEcheance[attributedEid] || 0) + Number(anyP.montant || 0);
        attributedLabels[pi] = label;
      }
    }
  }
  return (
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr>
              ...
              <th className="border px-2 py-1">Affecté à</th>
            </tr>
          </thead>
          <tbody>
            {paiements.map((p, i) => (
