...
export default function ConfigFrais() {
  ...
  const [frais, setFrais] = useState(() => {
    ...
    niveaux.forEach(niv => {
      ...
      obj[niv] = exist || {
        ...
        anneeScolaire,
        fraisInscription: 0,
        echeances: [{ date: '', montant: 0, modalite: 1, label: '' }],
      };
    });
    return obj;
  });
  ...
  const handleEcheanceChange = (idx: number, field: 'date' | 'montant' | 'label' | 'modalite', value: string | number) => {
    setFrais(prev => {
  ...
  const handleAddEcheance = () => {
    setFrais(prev => ({
      ...
      [selectedNiveau]: {
        ...prev[selectedNiveau],
        echeances: [...prev[selectedNiveau].echeances, { date: '', montant: 0, modalite: (prev[selectedNiveau].echeances || []).length + 1, label: '' }]
      }
    }));
  };
  const handleRemoveEcheance = (idx: number) => {
    setFrais(prev => {
      let echeances = prev[selectedNiveau].echeances.filter((_: any, i: number) => i !== idx);
      // reindex modalites to be consecutive
      echeances = echeances.map((e: any, i: number) => ({ ...e, modalite: i + 1 }));
      return {
        ...prev,
        [selectedNiveau]: {
          ...prev[selectedNiveau],
  ...
  const handleSave = () => {
    ...
    const payload: any = {
      ...
      anneeScolaire,
      echeances: (data.echeances || []).map((e: any, i: number) => ({ ...e, modalite: e.modalite || i + 1, id: e.id || `${selectedNiveau}-${i + 1}` })),
      fraisInscription: Number(data.fraisInscription || 0),
      createdAt: data.createdAt || new Date().toISOString(),
      updatedAt: new Date().toISOString()
    };

    // Validation stricte : modalites uniques
    const modalites = (payload.echeances || []).map((e: any) => Number(e.modalite));
    const uniqueModalites = new Set(modalites);
    if (uniqueModalites.size !== modalites.length) {
      alert('Les modalités des échéances doivent être uniques. Réindexez les échéances ou modifiez les modalités.');
      return;
    }
    // chaque échéance doit avoir un montant > 0
    const invalidE = (payload.echeances || []).find((e: any) => Number(e.montant || 0) <= 0);
    if (invalidE) {
      alert('Chaque échéance doit avoir un montant supérieur à 0.');
      return;
    }
    // somme totale > 0
    const sumE = (payload.echeances || []).reduce((s: number, x: any) => s + Number(x.montant || 0), 0);
    if (Number(payload.fraisInscription || 0) + sumE <= 0) {
      alert('Le montant total (inscription + échéances) doit être supérieur à 0.');
      return;
    }
    if (editingFraisId) {
  ...
  return (
    <FormLayout title="Configuration des Frais" subtitle={`Année active: ${anneeScolaire}`}>
            ...
            <tbody>
              {frais[selectedNiveau].echeances.map((ech: any, idx: number) => (
                  ...
                  <td className="border px-2 py-1 text-center">
                    <input type="number" className="w-16 px-1 py-1 border rounded text-center" value={ech.modalite || idx + 1} onChange={e => handleEcheanceChange(idx, 'modalite', Number(e.target.value))} />
                  </td>
                  <td className="border px-2 py-1">
                    <input type="text" value={ech.label || ''} onChange={e => handleEcheanceChange(idx, 'label', e.target.value)} className="px-2 py-1 border rounded w-full" />
                  </td>
                  <td className="border px-2 py-1">
