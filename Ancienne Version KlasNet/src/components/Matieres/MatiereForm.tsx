...
export default function MatiereForm({ matiere, onSave, onCancel }: MatiereFormProps) {
  ...
  bareme: (matiere as any).bareme || 20,
        type: matiere.type,
  ...
  if (formData.bareme <= 0) newErrors.bareme = 'Le barème doit être supérieur à 0';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (matiere) {
    const updatedMatiere = db.update<Matiere>('matieres', matiere.id, formData as any);
        if (updatedMatiere) {
          showToast('Matière mise à jour avec succès', 'success');
          onSave(updatedMatiere);
        }
      } else {
  const newMatiere = db.create<Matiere>('matieres', formData as any);
  ...
  return (
    <FormLayout title={matiere ? 'Modifier la matière' : 'Nouvelle matière'} subtitle={matiere ? 'Modifiez les informations de la matière' : 'Créez une nouvelle matière'}>
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Barème (ex: 50, 20) <span className="text-red-500">*</span>
              </label>
              <input
                type="number"
                value={(formData as any).bareme}
                onChange={(e) => handleInputChange('bareme', parseInt(e.target.value))}
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent ${
                  (errors as any).bareme ? 'border-red-300 bg-red-50' : 'border-gray-300'
                }`}
                min="1"
              />
              { (errors as any).bareme && <p className="mt-1 text-xs text-red-600">{(errors as any).bareme}</p> }
            </div>
          </div>
